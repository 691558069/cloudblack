...
- **审核流程** — 所有提交需管理员审核后才生效
- **AI 离线审核** — 夜间无人时段自动 AI 审核，自动拒绝垃圾/广告，自动通过多人举报
- **API 接口** — RESTful API，支持批量查询、快速检查
- **公开访问** — 查询/提交接口无需 API 密钥，受 RPM 限制保护
- **关联账号** — 支持同一主体下多个平台账号关联展示
- **风控系统** — IP 冷却期、原因字数检查、全局提交频率限制
- **访问日志** — 记录所有查询/提交操作，含来源、IP、API 密钥
- **管理后台** — 完整的 Web 管理面板，含仪表盘、日志、API 密钥管理、AI 设置
- **系统监控** — 实时 CPU、内存、数据库大小监控
- **多平台构建** — 支持 Windows/Linux/macOS，单文件部署
...
| `/api/web/stats` | GET | 前台统计数据（收录数/待审数） | 否 |
